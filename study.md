...
1. 객체 함수를 활용하여서 같은 성격의 함수들을 모아서 관리하자

> 재현 강사님 피드백

1. 유틸리티 함수로 객체에서 자주 호출하는 함수만 묶어주는 것이 좋을 것 같다.
2. 콜라의 정보를 각 객체 별로 묶어 관리하는 것이 좋을 것 같다.
    - 대부분의 경우 api를 통해 데이터를 내려받고, 그렇게 내려받는 데이터의 구조는 대부분 데이터를 객체 단위로 묶는다.
3. 객체 안에 프로퍼티를 만드는 것보다 주석으로 설명하는 것이 낫다.(프로퍼티도 메모리를 차지한다.)
